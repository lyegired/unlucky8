...
def edit_account(user_id):
    ...
    if session.get('user'):
        ...
        return render_template('editacc.html', account=my_account, user=session['user'], form = form, user_id = str(session['user_id']))
    else:
        return redirect(url_for('login'))

@app.route('/events/<event_id>/report', methods=['POST'])
